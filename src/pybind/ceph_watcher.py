...
class DaemonWatcher(object):
    ...
    def colorize(self, msg, color, dark=False):
        return (self.COLOR_DARK_SEQ if dark else self.COLOR_SEQ) % (30 + color) + msg + self.RESET_SEQ
    ...
    def format_dimless(self, n, width):
        units = [' ', 'k', 'M', 'G', 'T', 'P']
        unit = 0
        while len("%s" % (n / (1000**unit))) > width - 1:
            unit += 1
        ...
        if self._colored:
            if n == 0:
                color = self.BLACK, True
            else:
                color = self.YELLOW, False
            return self.bold(self.colorize(
                        formatted[0:-1], color[0], color[1])) \
                        + self.bold(self.colorize(formatted[-1], self.BLACK, True))
        else:
            return formatted
    ...
    def run(self):
        self._colored = self.supports_color()

        schema = json.loads(admin_socket(self.asok_path, ["perf", "schema"]))

        # Build list of which stats we will display, based on which
        # stats have a nickname
        stats = defaultdict(dict)
        for section_name, section_stats in schema.items():
            for name, schema_data in section_stats.items():
                if schema_data['nick']:
                    stats[section_name][name] = schema_data['nick']
        ...
        def print_headers():
            ...
            for section_name, names in stats.items():
                section_width = sum([len(x)+1 for x in names.values()]) - 1
            ...
            for section_name, names in stats.items():
                for stat_name, stat_nick in names.items():
                    sub_header += self.UNDERLINE_SEQ + self.colorize(stat_nick.ljust(self.col_width(stat_nick)), self.BLUE) + ' '
                sub_header = sub_header[0:-1] + self.colorize('|', self.BLUE)
            sub_header += "\n"
            ostr.write(sub_header)


        def print_vals(dump, last_dump):
            val_row = ""
            for section_name, names in stats.items():
                for stat_name, stat_nick in names.items():
                    COUNTER = 0x8

                    stat_type = schema[section_name][stat_name]['type']
                    is_counter = bool(stat_type & COUNTER)
                    if is_counter:
